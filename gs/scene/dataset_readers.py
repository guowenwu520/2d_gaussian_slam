...
def readColmapCameras(
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        if intr.model == "SIMPLE_PINHOLE":
            ...
            raise ValueError("Colmap camera model not handled: only PINHOLE or SIMPLE_PINHOLE supported")

        n_remove = len(extr.name.split('.')[-1]) + 1
        image_name_no_ext = extr.name[:-n_remove]
        ...
        if depths_params is not None:
            try:
                depth_params = depths_params[image_name_no_ext]
            except KeyError:

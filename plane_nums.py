...
def process_all_txt_files(folder_path):
    total_planes = 0
    file_plane_counts = []
    txt_files = [f for f in os.listdir(folder_path) if f.endswith('_data.txt')]
    max_planes = 0
    min_planes = 0
...
for scene in scenes:
   print(f"📂 正在处理 {scene} 数据集...")
   your_folder = f'/home/guowenwu/workspace/indoor_GS_SLAM/RGBD_GS_SLAM/datasets/replica/{scene}/plane'  # 替换为你的实际路径
   process_all_txt_files(your_folder)
